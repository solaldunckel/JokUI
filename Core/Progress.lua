-- local _, JokUI = ...
-- local MythicProgress = JokUI:RegisterModule("MythicProgress")
...
-- 	nameplateUpdateRate = 300, -- Rate (in milliseconds) at which we update the progress we get from the current pull, as estimated by active name plates you're in combat with. Also the update rate of getting new values for nameplate text overlay if enabled.
...
-- 	function exportData()
-- 		local a = string.format("Export ver %s (%s) - %i mobs: {",  tlen(JokUIDB["npcData"]))
